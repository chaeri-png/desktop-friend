...
- ✅ **3D 리디자인 완료** — 뱁새를 실물(흰머리오목눈이) 기반 Three.js 3D로 교체. 스펙: `docs/superpowers/specs/2026-07-31-baepsae-3d-design.md`
  ...
  - 3D 캐릭터 팩 = `characters/<이름>/character.json` + `model.js` (`createModel(container)` export). 현재 4종: 뱁새(과묵·다정·헤드셋), 햄스터(발랄·응원), 치즈냥(예의바른데 할 말은 함), 말티즈(애교 많은 해맑은 막내). 치즈냥·말티즈는 두 발로 서고 집중 시 책상에 앉아 직접 타이핑
